...
const Header = () => {
  ...
  return (
    <nav className="h-[92px] w-full top-[15px] z-50  sm:fixed sm:h-screen sm:w-screen  sm:z-50 md:h-[92px] md:w-full md:static ">
      <div className="container mx-auto px-4 py-3 flex justify-between items-center sm:flex-col sm:items-start sm:pt-8 md:flex-row md:items-center md:pt-0">
       
        <a href="#" className="text-[42px] font-darkline text-black italic sm:text-4xl md:text-[42px]">
          Sport News
        </a>

        
        <ul className="hidden md:flex space-x-6">
          <li><a href="#home" className="text-neutral-900 hover:text-gray-900 font-dmsans-medium">Home</a></li>
        ...
        <button className="hidden md:flex w-[100px] h-[35px] bg-g hover:bg-gray-500 text-white rounded-lg transition duration-300  items-center justify-center gap-2">
        ...
        <button onClick={() => setIsOpen(!isOpen)} className="md:hidden text-black sm:absolute sm:top-8 sm:right-8">
          ☰
        </button>
      </div>

      
      <div className={`${isOpen ? "block" : "hidden"} md:hidden bg-white fixed top-0 left-0 w-full h-screen z-50 flex flex-col justify-center items-center shadow-lg`}>
      
        <button onClick={() => setIsOpen(false)} className="absolute top-8 right-8 text-black text-2xl">
          ×
        </button>

        
        <ul className="flex flex-col items-center space-y-8">
          <li><a href="#home" onClick={() => setIsOpen(false)} className="text-neutral-900 hover:text-gray-900 font-dmsans-medium text-2xl">Home</a></li>
          <li><a href="#category" onClick={() => setIsOpen(false)} className="text-gray-800/60 hover:text-gray-900 font-dmsans-regular text-2xl">Category</a></li>
          <li><a href="#news" onClick={() => setIsOpen(false)} className="text-gray-800/60 hover:text-gray-900 font-dmsans-regular text-2xl">Trending News</a></li>
          <li><a href="#recent" onClick={() => setIsOpen(false)} className="text-gray-800/60 hover:text-gray-900 font-dmsans-regular text-2xl">Recent News</a></li>
          <li><a href="#ranking" onClick={() => setIsOpen(false)} className="text-gray-800/60 hover:text-gray-900 font-dmsans-regular text-2xl">Clubs Ranking</a></li>
          <li><a href="#sport" onClick={() => setIsOpen(false)} className="text-gray-800/60 hover:text-gray-900 font-dmsans-regular text-2xl">Sports Article</a></li>
        </ul>
      </div>
    </nav>
  );
